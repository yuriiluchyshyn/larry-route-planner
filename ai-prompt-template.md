...
Цей документ містить документацію для AI промпту оптимізації маршрутів.

**ВАЖЛИВО**: Для редагування промпту є два варіанти:
1. **Рекомендовано**: Редагуйте `src/config/aiPromptTemplate.md` (цей файл зручніший для редагування)
2. **Альтернативно**: Редагуйте `src/config/aiPromptTemplate.ts` (використовується в коді)

**Примітка**: Зміни в MD файлі потрібно вручну синхронізувати з TS файлом.
...
| Файл | Призначення |
|---|---|
| `src/config/aiPromptTemplate.md` | **Зручний для редагування** (markdown формат) |
| `src/config/aiPromptTemplate.ts` | **Використовується в коді** (експортує промпт) |
| `src/config/aiConfig.ts` | Імпортує промпт + налаштування AI |
| `ai-prompt-template.md` | Документація (цей файл) |
